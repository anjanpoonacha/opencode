...
// navigate-parent-button and subagent-indicator testids are not present in v1.18.6 timeline UI.
// Parent navigation is handled via breadcrumb in the session header instead.
test.skip("root session has no back arrow or subagent indicator", async ({ page, sdk, gotoSession }) => {
...
// navigate-parent-button testid not present in v1.18.6; parent navigation uses breadcrumb UI.
test.skip("subagent session shows back arrow and subagent indicator, and navigates to parent", async ({
  page,
  sdk,
  gotoSession,
}) => {
...
// navigate-parent-button testid not present in v1.18.6; parent navigation uses breadcrumb UI.
test.skip("navigating to parent session shows background only on parent", async ({ page, sdk, gotoSession }) => {
  await withSession(sdk, "parent session", async (parent) => {
    const child = await sdk.session.create({ title: "child session", parentID: parent.id }).then((r) => r.data)
    if (!child?.id) throw new Error("Failed to create child session")
